...
<p align="center">
  🌐 <b>Язык / Language:</b> <a href="README.md">English</a> • <b>Русский</b>
</p>

<p align="center">
  <img src="docs/screenshots/demo.gif" alt="CamView Демонстрация работы" width="100%" style="max-width: 820px; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.5);" />
</p>

<p align="center">
  <img src="docs/screenshots/desktop_grid.png" alt="Сетка мультикамерного наблюдения" width="49%" style="border-radius: 8px;" />
  <img src="docs/screenshots/network_modal.png" alt="Удаленный доступ по QR-коду" width="49%" style="border-radius: 8px;" />
</p>
...
### 🎭 Встроенный виртуальный Демо-режим
- Возможность протестировать все функции программы в 1 клик даже без подключённых физических веб-камер:
  - 🚗 **Камера 1 — Въезд и парковка**: Анимированное движение автомобиля, свет фар и рамка распознавания ИИ.
  - 🛋️ **Камера 2 — Гостиная / Холл**: Тикающие настенные часы с секундной стрелкой, уютный интерьер и датчик движения.
  - 🏊 **Камера 3 — Задний двор и бассейн**: Колышущиеся деревья, водная рябь бассейна и погодная телеметрия.
  - 🖥️ **Камера 4 — Офис и серверная**: Мигающие светодиоды серверных стоек, живые графики сетевого трафика.
- Генерирует настоящие MediaStream-потоки (30 FPS), доступные для записи в MP4, сохранения снимков, переключения сеток и трансляции на смартфон.
- Запуск кнопкой в шапке или параметром: `npm run demo` / `CamView.exe --demo`.
...
- **Pinggy (`*.pinggy.link` / `*.pinggy.net`)**: автоматический HTTPS-туннель в 1 клик через официальный SDK `@pinggy/pinggy` без лишней настройки, плюс поддержка прямого ввода поддомена 4-го уровня (`.a.free.pinggy.link`, `.a.pinggy.link`, `.free.pinggy.online`) с авто-распознаванием при вставке полного URL.
...
├── README.md                # Английская документация проекта
├── README_ru.md             # Русскоязычная документация проекта
